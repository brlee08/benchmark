...
#LOGs
TRAIN_DIR=/gpfs/gpfs_gl4_16mb/b7p284za/benchmark/tensorflow/train_log/googlenet-10e-128b-8G
SUMMARY_VERBOSITY=1
...
NUM_EPOCHS=10
NUM_GPU=8
NUM_BATCHES=$((${SAMPLE_SIZE} * ${NUM_EPOCHS} / 128 / ${NUM_GPU}))
#NUM_BATCHES=1000
...
W_HOST=(129.40.42.108 129.40.42.114)
...
train() {
    ...
    TASK_INDEX=$4
    TIMESTAMP=$(date +%m%d%H%M)
    LOG_FILE="${LOG_DIR}/output_tid${TASK_INDEX}_${MODEL}_e${NUM_EPOCHS}_b${BATCH_SIZE}.${TIMESTAMP}_ps.log"
    python tf_cnn_benchmarks/tf_cnn_benchmarks.py \
        --job_name=${JOB_NAME} \
        --ps_hosts="${PS_HOST[0]}:${PORT_PS}" \
        --worker_hosts="${W_HOST[0]}:${PORT_WORKER},${W_HOST[1]}:${PORT_WORKER}" \
...
#train resnet101 96 ps 0
train googlenet 128 ps 0
